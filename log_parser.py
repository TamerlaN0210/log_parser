    ...
    def parse(self, array: list, parse_type: str):
        if parse_type == 'watch':
            ...
            if array[-2] not in Parser.ips:
                ...
                query = str()
                query = ''.join([query, 'INSERT INTO `ip`(`adress1`,`adress2`,`adress3`,`adress4`) ',
                                 'VALUES ("', ip[0], '","', ip[1], '","', ip[2], '","', ip[3], '");'])
                self.mycursor.execute(query)
                self.mydb.commit()
            query = str()
            query = ''.join([query, 'INSERT INTO `actions`(`datetime`, `hash`, `ip`, `type`,`section`,`good`) ',
                             'VALUES ("', ' '.join([array[2], array[3]]), '","', array[4], '",',
        ...
        if parse_type == 'add':
            ...
            query = str()
            query = ''.join([query, 'UPDATE `goods` SET `id_from_logs` = "', add_info[0][1], '" ',
                             'WHERE `id` = "', good_id, '";'])
            self.mycursor.execute(query)
            self.mydb.commit()
            ...
            query = str()
            query = ''.join([query, 'INSERT INTO ',
                             '`actions`(`datetime`,`hash`,`ip`,`type`,`id_goods_in_carts`,`id_carts`)',
                             'VALUES ("', ' '.join([array[2], array[3]]), '","', array[4], '",',
                             '(SELECT `id` FROM `ip` WHERE `adress1` = "', ip[0], '" ',
                             'AND `adress2` = "', ip[1], '" AND `adress3` = "', ip[2], '" ',
                             ...
                             '(SELECT `id` FROM `carts` WHERE `cart_id` = "', add_info[2][1], '"))'])
            # последний ID был из запроса выше, т.е. id в goods_in_carts
            self.mycursor.execute(query)
            self.mydb.commit()
            # print('parser_add')
        if parse_type == 'pay':
            ...
            if pay_info[0][1] not in Parser.users:
                ...
                self.mycursor.execute(query)
                self.mydb.commit()
            query = ''.join(['UPDATE `carts` ',
                             'SET `user_id`=(SELECT `id` FROM `users` WHERE `user_id`= "', pay_info[0][1], '") ',
                             'WHERE `cart_id`="', pay_info[1][1], '" ',
                             'AND `user_id` IS NULL;'])
            self.mycursor.execute(query)
            self.mydb.commit()
            query = ''.join(['INSERT INTO `actions`(`datetime`, `hash`, `ip`, `type`,`id_carts`) VALUES ("',
                             ...
                             '(SELECT `id` FROM `ip` WHERE `adress1` = "', ip[0], '" ',
                             'AND `adress2` = "', ip[1], '" AND `adress3` = "', ip[2], '" ',
                             'AND `adress4` = "', ip[3], '"), "pay", ',
                             '(SELECT `id` FROM `carts` WHERE `cart_id`="', pay_info[1][1], '"));'])
            self.mycursor.execute(query)
            self.mydb.commit()
            # print('parser_pay')
        if parse_type == 'confirm_pay':
            ...
            query = ''.join(['INSERT INTO `actions`(`datetime`, `hash`, `ip`, `type`,`id_carts`) VALUES ("',
                             ...
                             '(SELECT `id` FROM `ip` WHERE `adress1` = "', ip[0], '" ',
                             'AND `adress2` = "', ip[1], '" AND `adress3` = "', ip[2], '" ',
                             'AND `adress4` = "', ip[3], '"), "confirm_pay",',
                             '(SELECT `id` FROM `carts` WHERE `cart_id`="', cart_number, '"));'])
            self.mycursor.execute(query)
            self.mydb.commit()
    ...
    def read(self):
        ...
        for line in file:
            ...
            if temp[-1].find('success_pay_') != -1:
                ...
                self.parse(temp, 'pay')
            if counter % 1000 == 0:
                print('Обработано {} строк'.format(counter))
